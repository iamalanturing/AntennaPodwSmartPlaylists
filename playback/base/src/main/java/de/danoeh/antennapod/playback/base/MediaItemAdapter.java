...
public class MediaItemAdapter {
    ...
    public static MediaItem fromPlayable(Context context, Playable playable) {
        ...
        Bundle requestExtras = Bundle.EMPTY;
        if (playable instanceof FeedMedia && !((FeedMedia) playable).localFileAvailable()) {
            FeedMedia feedMedia2 = (FeedMedia) playable;
            if (feedMedia2.getItem() != null && feedMedia2.getItem().getFeed() != null) {
                FeedPreferences prefs = feedMedia2.getItem().getFeed().getPreferences();
                if (prefs != null && !TextUtils.isEmpty(prefs.getUsername()) && !TextUtils.isEmpty(prefs.getPassword())) {
                    requestExtras = new Bundle();
